import {isValidDocumentId, type AnyDocumentId} from "@automerge/automerge-repo/slim"
...
self.addEventListener('install', (event) => {
	...
	self.skipWaiting()
	event.waitUntil(caches.open("everything").then((cache) => {
...
self.addEventListener("fetch", (event: FetchEvent) => {
	...
	let [, documentId, ext] = url.pathname.match(/\/([A-Za-z0-9]+)\.([A-Za-z0-9]+)/) ?? []


	if (documentId && ext && isValidDocumentId(documentId)) {
		try {
			let handle = repo.find<{text: string}>(documentId as AnyDocumentId)
			event.respondWith(handle.doc().then(async doc => {

				await repo.networkSubsystem.whenReady()
				return new Response(doc?.text, {
					headers: {
						"content-type": types.find(([r]) => r.exec(url.pathname))?.[1] ?? "text/plain"
					},
					status: 200,
					statusText: "yay!",
				})
